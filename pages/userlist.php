...
if ($uz==$data[2]){
echo '<b><big><a href="../pages/anketa.php?uz='.$data[2].'"><span style="color:#ff0000">'.nickname($data[2]).'</span></a></big></b> (Баллов: <b>'.$data[3].'</b>)</div>';
} else {
echo '<b><a href="../pages/anketa.php?uz='.$data[2].'">'.nickname($data[2]).'</a></b> (Баллов: <b>'.$data[3].'</b>)</div>';
}
...
echo '<form action="userlist.php?start='.$start.'" method="post">';
...
if ($start!=$stranica){
header ("Location: userlist.php?start=$stranica&uz=$uz"); exit;
}
...
echo '<br /><a href="userlist.php?start='.$start.'">Искать еще</a><br />';
