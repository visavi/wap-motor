...
echo '<form action="add.php?action=add&amp;id='.$id.'" method="post">';
...
antiflood("Location: add.php?id=$id&isset=antiflood");
karantin($udata[6], "Location: add.php?id=$id&isset=karantin");
...
header ("Location: index.php?action=board&id=$id&isset=addboard");	exit;
...
echo '<br /><img src="../images/img/back.gif" alt="image" /> <a href="add.php?id='.$id.'">Вернуться</a>';
}

} else {show_login('Вы не авторизованы, чтобы добавить объявление, необходимо');}

echo '<br /><img src="../images/img/reload.gif" alt="image" /> <a href="index.php">Объявления</a><br />';
