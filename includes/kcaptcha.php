...
class KCAPTCHA{
    ...
    function __construct(){ // new PHP declaration
        ...
        # folder with fonts
        $fontsdir = '../gallery/captcha';

        # CAPTCHA string length
        $length = mt_rand(4, 5); # random 4 or 6
        ...
        $white_noise_density = ($config['protectdef']) ? 1/10 : 0;
        //$black_noise_density=0; // no black noise
        $black_noise_density = ($config['protectdef']) ? 1/30 : 0;

        # increase safety by prevention of spaces between symbols
        $spaces = $config['protectdef'] ? false : true;

        # show credits
        $show_credits = ''; # set to false to remove credits line. Credits adds 12 pixels to image height
