...
if ($key==0){
echo '<a href="antimat.php?action=del&amp;del='.$key.'&amp;uid='.$_SESSION['token'].'&amp;'.SID.'">'.$data[0].'</a>';	
} else {
echo ', <a href="antimat.php?action=del&amp;del='.$key.'&amp;uid='.$_SESSION['token'].'&amp;'.SID.'">'.$data[0].'</a>';
}}
...
echo '<form action="antimat.php?action=add&amp;uid='.$_SESSION['token'].'&amp;'.SID.'" method="post">';
...
echo '<img src="../images/img/error.gif" alt="image" /> <b><a href="antimat.php?action=alldel&amp;uid='.$_SESSION['token'].'&amp;'.SID.'">Да уверен!</a></b><br />';	
...
header ("Location: antimat.php?isset=mp_clearmat&".SID); exit;
