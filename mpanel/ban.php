...
echo '<form action="ban.php?action=del&amp;start='.$start.'&amp;uid='.$_SESSION['token'].'&amp;'.SID.'" method="post">';
...
echo '<b><a href="../pages/anketa.php?uz='.$data[3].'&amp;'.SID.'">'.nickname($data[3]).'</a></b><br />';
}
...
echo '<hr /><form method="post" action="ban.php?action=add&amp;start='.$start.'&amp;uid='.$_SESSION['token'].'&amp;'.SID.'">';
echo 'IP-адрес:<br />';
...
if ($total>1){echo'<img src="../images/img/error.gif" alt="image" /> <a href="ban.php?action=clear&amp;uid='.$_SESSION['token'].'&amp;'.SID.'">Очистить список</a><br />';}
...
header ("Location: ban.php?start=$start&isset=mp_addbanlist&".SID); exit;
...
header ("Location: ban.php?start=$start&isset=mp_delbanlist&".SID);  exit;
...
header ("Location: ban.php?start=$start&isset=mp_clearbanlist&".SID); exit;
