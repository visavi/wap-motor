...
echo '<form action="navigation.php?action=del&amp;uid='.$_SESSION['token'].'&amp;'.SID.'" method="post">';

foreach($files as $key=>$val){
$data = explode("|", $val);	

echo '<div class="b">';
echo '<img src="../images/img/edit.gif" alt="image" /> <b><a href="navigation.php?action=edit&amp;id='.$key.'&amp;'.SID.'">'.$data[1].'</a></b> ('.$data[0].')</div>';

echo '<div><input type="checkbox" name="del[]" value="'.$key.'" /> ';

if ($key != 0){echo '<a href="navigation.php?action=move&amp;id='.$key.'&amp;where=0&amp;uid='.$_SESSION['token'].'&amp;'.SID.'">Вверх</a> | ';} else {echo 'Вверх | ';}
if ($total > ($key+1)){echo '<a href="navigation.php?action=move&amp;id='.$key.'&amp;where=1&amp;uid='.$_SESSION['token'].'&amp;'.SID.'">Вниз</a>';} else {echo 'Вниз';}
...
echo '<br /><img src="../images/img/edit.gif" alt="image" /> <a href="navigation.php?action=add&amp;'.SID.'">Добавить</a>';
}
...
echo '<form action="navigation.php?id='.$id.'&amp;action=addedit&amp;uid='.$_SESSION['token'].'&amp;'.SID.'" method="post">';
...
header ("Location: navigation.php?isset=mp_editnavigation&".SID); exit;
...
echo '<form action="navigation.php?action=addstr&amp;uid='.$_SESSION['token'].'&amp;'.SID.'" method="post">';
...
header ("Location: navigation.php?isset=mp_addnavigation&".SID); exit;
...
header ("Location: navigation.php?isset=mp_movenavigation&".SID); exit;
...
header ("Location: navigation.php?isset=mp_delnavigation&".SID); exit;
